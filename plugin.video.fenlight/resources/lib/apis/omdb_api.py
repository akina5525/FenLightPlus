...
class OMDbAPI:
	...
	def process_result(self, imdb_id, meta):
		...
		logger("omdb_api.py", f'process_result - final data being cached (just before adding to meta): {data}')
	...
	def get_result(self, imdb_id):
		try:
			...
			logger("omdb_api.py", f'get_result - raw text response from OMDb: {result}')
			...
			logger("omdb_api.py", f'get_result - EXCEPTION: {e}')
			return None
